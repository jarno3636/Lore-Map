...
/**
 * These addresses are carried forward from the existing Atlas configuration.
 * Treat them as deployment configuration: verify each one against official
 * Tobyworld sources before relying on them for production wallet reads.
 */
...
} satisfies TobyworldAssetDefinition;
...
export const TOBYWORLD_ASSET_BY_ID: Record<
  TobyworldAssetId,
  TobyworldAssetDefinition
> = {
  toby: tobyAsset,
  patience: patienceAsset,
  taboshi: taboshiAsset,
};
