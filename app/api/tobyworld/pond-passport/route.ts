...
type PassportRow = {
  ...
  reroll_count: number;
  snapshot: PondPassportSnapshot | Record<string, unknown>;
};
...
function rowToPersona(row: PassportRow): PondPassportPersona {
  return {
    title: row.title,
    characteristic: row.characteristic,
    strangeHabit: row.strange_habit,
    pondWarning: row.pond_warning,
    stamp: row.stamp,
...
${JSON.stringify({
  ...
  totalEchoes: snapshot.totalEchoes,
  totalRites: snapshot.totalRites,
  randomizer: nonce,
})}
`.trim();
  ...
  if (!response.ok) {
    ...
    return {
      persona: fallback,
      source: 'fallback:gemini_http_error',
    };
  }
  ...
  if (!text) {
    return {
      persona: fallback,
      source: 'fallback:empty_gemini_response',
    };
  }
  ...
  try {
    ...
    return {
      persona: fallback,
      source: 'fallback:json_parse_failed',
    };
  }
}
    ...
    .upsert(
      {
        ...
        strange_habit: persona.strangeHabit,
        pond_warning: persona.pondWarning,
        stamp: persona.stamp,
        share_text: persona.shareText,
        seed,
        source,
        generated_on: today,
...
async function createPassport(
  ...
  const nextRerollCount = reroll
    ? existing?.generated_on === today
      ? existing.reroll_count + 1
      : 1
    : 0;
